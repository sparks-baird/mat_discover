...
def njit_cdf_distance(
    ...
    # allocate local float arrays
    # combined vector
    uv = np.zeros(tot_cols, nb_float)
    uv_deltas = np.zeros(tot_cols_minus_1, nb_float)

    # CDFs
    u_cdf = np.zeros(tot_cols_minus_1, nb_float)
    v_cdf = np.zeros(tot_cols_minus_1, nb_float)

    # allocate local int arrays
    # CDF indices via binary search
    u_cdf_indices = np.zeros(tot_cols_minus_1, nb_int)
    v_cdf_indices = np.zeros(tot_cols_minus_1, nb_int)

    u_cdf_sorted_cumweights = np.zeros(tot_cols_minus_1, nb_float)
    v_cdf_sorted_cumweights = np.zeros(tot_cols_minus_1, nb_float)
    ...
    # sorting, accumulating, and prepending (for compatibility)
        ...
        if not presorted:
            # local arrays
            u_sorted = np.zeros(cols, np_float)
            v_sorted = np.zeros(cols, np_float)

            u_sorter = np.zeros(cols, nb_int)
            v_sorter = np.zeros(cols, nb_int)

            u_sorted_weights = np.zeros(cols, nb_float)
            v_sorted_weights = np.zeros(cols, nb_float)
        ...
        if not cumweighted:
            # local arrays
            u_cumweights = np.zeros(cols, nb_float)
            v_cumweights = np.zeros(cols, nb_float)
            # accumulate
            hp.cumsum(u_sorted_weights, u_cumweights)
            hp.cumsum(v_sorted_weights, v_cumweights)
        ...
        # prepend weights with zero
        if not prepended:
            zero = np.zeros(1, nb_float)

            u_0_cumweights = np.zeros(cols_plus_1, nb_float)
            v_0_cumweights = np.zeros(cols_plus_1, nb_float)
